...
1. Создайте каталог для кеша и выдайте права пользователю `zabbix` (если используете путь по умолчанию):
...
1. Создайте файл конфигурации по образцу:
...
1. Убедитесь, что пользователь `zabbix` может писать в `LOG_DIR` (по умолчанию `/var/log/zabbix`), либо укажите в env-файле другой каталог с правами на запись.
