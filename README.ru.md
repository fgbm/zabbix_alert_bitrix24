...
Bash-скрипт для отправки уведомлений Zabbix в чат Bitrix24 **от имени чат-бота**: REST `imbot.v2.Bot.register` + `imbot.v2.Chat.Message.send` через входящий вебхук (scope `imbot`). Поддерживается **маршрутизация по пользователю** через `{ALERT.SENDTO}` в Zabbix (рекомендуется) или дефолтный `BITRIX_DIALOG_ID` в env; сохранена совместимость со **старым вызовом в два аргумента** (только тема и сообщение).
...
- **Добавьте бота в нужный групповой чат** как участника — иначе отправка в `chat…` завершится ошибкой доступа. В личный диалог пишите, указывая в **Send to** или в `BITRIX_DIALOG_ID` **только числовой ID пользователя** (без префикса `chat`).
...
3. Создайте файл конфигурации по образцу:
  ...
  ```
   Отредактируйте `/etc/zabbix/bitrix_alerts.env`: задайте `BITRIX_WEBHOOK_URL`, `BITRIX_BOT_CODE`, `BITRIX_BOT_TOKEN` и при необходимости имя/должность бота. `BITRIX_DIALOG_ID` задавайте как **маршрут по умолчанию**, если не используете поле **Send to** у каждого пользователя (см. раздел *Способ оповещения в Zabbix*).
...
### Маршрутизация (чат / dialog_id)

- **Рекомендуется:** в оповещениях пользователя Zabbix в поле **Отправить на** (**Send to**) укажите Bitrix `dialog_id` (например `chat123` для группового чата или **числовой** ID пользователя для лички — без префикса `chat`). В поставляемом YAML первым параметром передаётся `{ALERT.SENDTO}`; скрипт использует его как `dialog_id`, если после обрезки пробелов строка не пустая.
- **Запасной вариант:** если **Send to** пустой, используется `BITRIX_DIALOG_ID` из `/etc/zabbix/bitrix_alerts.env` (один чат по умолчанию для этой записи media).
- **Legacy:** раньше передавались только `{ALERT.SUBJECT}` и `{ALERT.MESSAGE}`; это по-прежнему поддерживается: `dialog_id` берётся **только** из `BITRIX_DIALOG_ID` в env. Явный импорт: [assets/zabbix/zabbix_bitrix24_mediatype_legacy_2params.yaml](assets/zabbix/zabbix_bitrix24_mediatype_legacy_2params.yaml).

### Импорт готового файла (рекомендуется)

В репозитории есть YAML для импорта через веб-интерфейс: [assets/zabbix/zabbix_bitrix24_mediatype.yaml](assets/zabbix/zabbix_bitrix24_mediatype.yaml) (экспорт с `version: '6.0'`, подходит для импорта в Zabbix 6.0 / 6.4 / 7.x).

1. Откройте **Способы оповещений** (**Media types**): в Zabbix 7.x — **Предупреждения** → **Способы оповещений**; в Zabbix 6.0 — **Администрирование** → **Способы оповещений**.
2. Нажмите **Импорт** (**Import**), выберите файл `zabbix_bitrix24_mediatype.yaml` (скачайте из репозитория или по raw-URL с GitHub).
3. В правилах импорта отметьте **Создать новое** (**Create new**); при обновлении существующего способа — также **Обновить существующее** (**Update existing**).
4. Нажмите **Импорт** (**Import**).

Дальше создайте **пользователя Zabbix** (или используйте существующего), которому в **Оповещения** добавьте этот способ: в **Send to** укажите целевой `dialog_id` (если не полагаетесь на дефолтный `BITRIX_DIALOG_ID` в env). На этого пользователя настройте **действие** (**Action**) — именно в нём задаются шаблоны темы и сообщения.

Дополнительные параметры **EVENT.\*** в YAML заполняются макросами Zabbix для трассировки в syslog; чтобы дублировать их в тексте чата, задайте в env `BITRIX_APPEND_EVENT_METADATA=1` (см. [bitrix_alerts.env.example](bitrix_alerts.env.example)).

### Миграция со старого способа (2 параметра)

1. Импортируйте новый [zabbix_bitrix24_mediatype.yaml](assets/zabbix/zabbix_bitrix24_mediatype.yaml) (или отредактируйте существующий Script media type вручную).
2. Для каждой записи **Оповещения** пользователя укажите **Send to** с нужным `dialog_id` (или оставьте пустым, если устраивает один `BITRIX_DIALOG_ID` из env).
3. Проверьте доставку через **Тест** в способе оповещения или тестовое событие.

### Ручная настройка

Если импорт недоступен или не подходит:

**Создать способ оповещения** (рекомендуемый порядок параметров):
...
- **Параметры скрипта** (важен порядок):
  ...
  2. `{ALERT.SUBJECT}`
  3. `{ALERT.MESSAGE}`
  4. `{EVENT.SOURCE}` (опционально; syslog / опциональный хвост в сообщении)
  5. `{EVENT.VALUE}`
  6. `{EVENT.SEVERITY}`

**Legacy** (по-прежнему поддерживается; нужен `BITRIX_DIALOG_ID` в env):

1. `{ALERT.SUBJECT}`
2. `{ALERT.MESSAGE}`

Дальше — назначение способа пользователю и настройка действия, как выше.

Скрипт оформляет сообщение для чата Bitrix24: **жирный** заголовок (тема) и текст тела.

### Доп. настройки env (эксплуатация)

См. [bitrix_alerts.env.example](bitrix_alerts.env.example): `BITRIX_APPEND_EVENT_METADATA`, `BITRIX_STRICT_DIALOG_ID`, `BITRIX_MESSAGE_MAX_LEN`, `BITRIX_CURL_MAX_TIME`, `BITRIX_CURL_RETRIES`, `BITRIX_CURL_RETRY_DELAY`.
...
В шаблонах действий Zabbix в тему или текст можно добавлять другие поддерживаемые теги, например `[I]…[/I]`, `[URL=https://example.com]подпись[/URL]`, перенос `[BR]` или обычный `\n`. Регистр тегов в примерах документации часто смешанный (`[b]` и `[B]`); ориентируйтесь на официальный список — произвольный «интернет-BBCode» может не сработать. Максимальная длина текста сообщения в API — **20 000** символов (лишнее Bitrix24 обрежет). Скрипт обрезает текст до `BITRIX_MESSAGE_MAX_LEN` (по умолчанию 20000) перед отправкой и пишет предупреждение в syslog при обрезке.
...
- **Коды выхода**: `2` — нет конфигурации, нет `jq`/`curl`, неверное число аргументов, пустые тема и сообщение, не задан `dialog_id`, либо неверный `dialog_id` при `BITRIX_STRICT_DIALOG_ID=1`; `1` — ошибка Bitrix24/curl или сеть (в Zabbix доставка должна отображаться как неуспешная).
- **Пустой маршрут:** если ошибка «dialog_id is empty», задайте **Send to** у пользователя или `BITRIX_DIALOG_ID` в env.
