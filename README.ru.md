...
Дальше создайте **пользователя Zabbix** (или используйте существующего), которому в **Оповещения** добавьте этот способ с любым `Send to` (используется как маркер, скрипт его не читает) и нужным расписанием. На этого пользователя настройте **действие** (**Action**) — именно в нём задаются шаблоны темы и сообщения.
